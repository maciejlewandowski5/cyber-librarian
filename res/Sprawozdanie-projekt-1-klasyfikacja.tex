...
Metoda $k$-NN służy do klasyfikacji obiektów. Opiera się na założeniu podobieństwa obiektów blisko położonych w przestrzeni cech. Algorytm składa się z następujących kroków:
\begin{enumerate}
\item Weź obiekt do klasyfikacji i znajdź $k$ najbliższych sąsiadów ze zbioru uczącego
\item Zdeterminuj klasę badanego obiektu jako klasę większości instancji spośród $k$ najbliższych sąsiadów.
\end{enumerate}
Odległość dwóch obiektów określana jest poprzez określoną metrykę. Porównywane będą wektory cech reprezentujące obiekty.
\indent Algorytm wymaga na wejściu określenia: liczby sąsiadów, metryki, podziału zbioru uczącego. Rezultatem działania algorytmu $k$-NN są sklasyfikowane obiekty. 
...
\item Względna częstość występowania najczęściej pojawiającej się nazwy morza lub oceanu
\begin{equation}
v_{6}=\frac{max(k(A,G_M))}{v_1}\label{eq:eps}
\end{equation}
,gdzie
